local vfs = App:get_vfs()
local WORKING_DIR = vfs:is_mounted_dir(vfs:PROJECT_DIR()) and vfs:PROJECT_DIR() or vfs:APP_DIR()

local Config = require_script(WORKING_DIR, "Scripts/config.lua")

local NetworkController = {
  server = nil,
  client = nil
}

NetworkController.Mode = {
  SinglePlayer = 1,
  Host = 2,
  Client = 3
}
NetworkController.current_mode = NetworkController.Mode.SinglePlayer

-- Assigned by ui.lua / scene.lua.
NetworkController.on_peer_connected = nil
NetworkController.on_peer_disconnected = nil
NetworkController.on_lobby_update = nil
NetworkController.on_match_start = nil
NetworkController.on_score = nil
NetworkController.on_round_reset = nil
NetworkController.on_session_end = nil

-- Lobby state. Player 1 is always the host, player 2 whoever joined.
NetworkController.peer_connected = false
NetworkController.local_ready = false
NetworkController.remote_ready = false

local peer_client_id = nil
-- Latest paddle direction received from the client, host only.
local remote_input_dir = 0
-- Latest world state received from the host, client only.
local latest_state = nil

-- init() subscriptions live for as long as the online menu is open, session ones only for as long
-- as a server is up.
local base_subscriptions = {}
local session_subscriptions = {}

local function track(list, subscription)
  if subscription then
    list[#list + 1] = subscription
  else
    Oxlog.error("Failed to subscribe to a network event.")
  end
end

local function clear_subscriptions(list)
  for _, subscription in ipairs(list) do
    subscription:unsubscribe()
  end
  for i = #list, 1, -1 do
    list[i] = nil
  end
end

-- Disconnect events are emitted from inside enet's service loop, so hosts can never be destroyed
-- straight from a handler. Teardown always waits for the next tick.
local pending_client_teardown = false
local pending_server_teardown = false

function NetworkController.is_host()
  return NetworkController.current_mode == NetworkController.Mode.Host
end

function NetworkController.is_client()
  return NetworkController.current_mode == NetworkController.Mode.Client
end

function NetworkController.is_online()
  return NetworkController.current_mode ~= NetworkController.Mode.SinglePlayer
end

-- Returns the ready flags in player order, whichever side we happen to be on.
function NetworkController.ready_states()
  if NetworkController.is_client() then
    return NetworkController.remote_ready, NetworkController.local_ready
  end
  return NetworkController.local_ready, NetworkController.remote_ready
end

function NetworkController.both_ready()
  local p1_ready, p2_ready = NetworkController.ready_states()
  return NetworkController.peer_connected and p1_ready and p2_ready
end

local function reset_lobby()
  NetworkController.peer_connected = false
  NetworkController.local_ready = false
  NetworkController.remote_ready = false
  peer_client_id = nil
  remote_input_dir = 0
  latest_state = nil
end

local function fire(callback, ...)
  if callback then
    callback(...)
  end
end

local function broadcast_lobby()
  if not NetworkController.server then
    return
  end

  NetworkController.server:broadcast("lobby", {
    NetworkController.local_ready and 1 or 0,
    NetworkController.remote_ready and 1 or 0,
  }, true)
end

local function session_end(reason)
  fire(NetworkController.on_session_end, reason)
end

function NetworkController.init()
  -- The online menu can be entered more than once per run, so never stack subscriptions.
  clear_subscriptions(base_subscriptions)

  local es = App:get_event_system()

  track(base_subscriptions, es:subscribe_server_connect_event(function(e)
    Oxlog.info("Connected to server. NetID: " .. e.net_id)

    NetworkController.current_mode = NetworkController.Mode.Client
    NetworkController.peer_connected = true

    fire(NetworkController.on_peer_connected)
  end))

  track(base_subscriptions, es:subscribe_server_disconnect_event(function(e)
    pending_client_teardown = true
    reset_lobby()

    if e.reason == NetClientStatus.TimedOut then
      Oxlog.warn("Connection attempt timed out.")
      session_end("Could not reach host.")
    else
      Oxlog.info("Lost connection to server.")
      session_end("Disconnected from host.")
    end
  end))
end

function NetworkController.deinit()
  clear_subscriptions(base_subscriptions)
  clear_subscriptions(session_subscriptions)
  reset_lobby()

  NetworkController.destroy_client()
  NetworkController.destroy_server()
end

local function register_host_procs(server)
  server:register_proc("ready", function(client_id, params)
    NetworkController.remote_ready = (params[1] or 0) ~= 0

    broadcast_lobby()
    fire(NetworkController.on_lobby_update)
  end)

  server:register_proc("input", function(client_id, params)
    remote_input_dir = params[1] or 0
  end)
end

local function register_client_procs(client)
  client:register_proc("lobby", function(_, params)
    NetworkController.remote_ready = (params[1] or 0) ~= 0
    NetworkController.local_ready = (params[2] or 0) ~= 0

    fire(NetworkController.on_lobby_update)
  end)

  client:register_proc("start", function()
    fire(NetworkController.on_match_start)
  end)

  client:register_proc("state", function(_, params)
    latest_state = {
      p1_y = params[1] or 0,
      p2_y = params[2] or 0,
      ball_x = params[3] or 0,
      ball_y = params[4] or 0,
      ball_vx = params[5] or 0,
      ball_vy = params[6] or 0,
    }
  end)

  client:register_proc("score", function(_, params)
    fire(NetworkController.on_score, params[1] or 0, params[2] or 0, params[3] or 0)
  end)

  client:register_proc("round", function()
    fire(NetworkController.on_round_reset)
  end)
end

function NetworkController.start_host(port)
  local nm = App.mod.NetworkManager
  local es = App:get_event_system()

  NetworkController.server = nm:create_server(port, Config.NET.MAX_CLIENTS)
  if not NetworkController.server then
    Oxlog.error("Failed to host on port " .. port .. ".")
    return false
  end

  NetworkController.server:set_tick_rate(Config.NET.TICK_RATE)
  NetworkController.current_mode = NetworkController.Mode.Host
  reset_lobby()

  register_host_procs(NetworkController.server)

  track(session_subscriptions, es:subscribe_client_connect_event(function(e)
    Oxlog.info("Client connected " .. e.client_id)

    peer_client_id = e.client_id
    NetworkController.peer_connected = true
    NetworkController.remote_ready = false

    -- The joiner has no idea what the host already toggled, tell it.
    broadcast_lobby()
    fire(NetworkController.on_peer_connected)
  end))

  track(session_subscriptions, es:subscribe_client_disconnect_event(function(e)
    Oxlog.info("Client disconnected " .. e.client_id)

    peer_client_id = nil
    NetworkController.peer_connected = false
    NetworkController.remote_ready = false
    remote_input_dir = 0

    fire(NetworkController.on_peer_disconnected, "Opponent left the game.")
  end))

  return true
end

function NetworkController.connect_to_server(ip, port)
  local nm = App.mod.NetworkManager

  NetworkController.destroy_client()
  NetworkController.client = nm:create_client()
  if not NetworkController.client then
    Oxlog.error("Failed to create a client.")
    return false
  end

  NetworkController.client:set_tick_rate(Config.NET.TICK_RATE)
  reset_lobby()

  -- Procs have to exist before the first packet lands, so register before connecting.
  register_client_procs(NetworkController.client)

  if not NetworkController.client:connect(ip, port, Config.NET.CONNECT_TIMEOUT_MS) then
    Oxlog.error("Failed to start connecting to " .. ip .. ":" .. port .. ".")
    NetworkController.destroy_client()
    return false
  end

  return true
end

function NetworkController.destroy_server()
  local nm = App.mod.NetworkManager
  pending_server_teardown = false
  NetworkController.current_mode = NetworkController.Mode.SinglePlayer
  if NetworkController.server then
    nm:destroy_server(NetworkController.server)
  end
  NetworkController.server = nil
end

function NetworkController.destroy_client()
  local nm = App.mod.NetworkManager
  pending_client_teardown = false
  if NetworkController.client then
    nm:destroy_client(NetworkController.client)
  end
  NetworkController.client = nil
end

-- Leaves the current session but keeps the online menu subscriptions alive. Safe to call from a
-- network event handler, the actual hosts go away on the next tick.
function NetworkController.leave()
  clear_subscriptions(session_subscriptions)
  reset_lobby()

  pending_client_teardown = NetworkController.client ~= nil
  pending_server_teardown = NetworkController.server ~= nil

  NetworkController.current_mode = NetworkController.Mode.SinglePlayer
end

function NetworkController.set_local_ready(is_ready)
  if not NetworkController.is_online() then
    return
  end

  NetworkController.local_ready = is_ready

  if NetworkController.is_host() then
    broadcast_lobby()
  elseif NetworkController.client then
    NetworkController.client:call_server("ready", { is_ready and 1 or 0 }, true)
  end

  fire(NetworkController.on_lobby_update)
end

function NetworkController.request_start_match()
  if not NetworkController.is_host() or not NetworkController.both_ready() then
    return false
  end

  NetworkController.server:broadcast("start", {}, true)
  fire(NetworkController.on_match_start)

  return true
end

function NetworkController.send_input(dir)
  if NetworkController.client then
    NetworkController.client:call_server("input", { dir }, false)
  end
end

function NetworkController.remote_input()
  return remote_input_dir
end

-- Returns the state only once per update received. Applying the same snapshot every frame would
-- pin the ball in place instead of letting it dead reckon between ticks.
function NetworkController.consume_state()
  local state = latest_state
  latest_state = nil

  return state
end

function NetworkController.broadcast_state(p1_y, p2_y, ball_position, ball_velocity)
  if not NetworkController.server then
    return
  end

  NetworkController.server:broadcast("state", {
    p1_y,
    p2_y,
    ball_position.x,
    ball_position.y,
    ball_velocity.x,
    ball_velocity.y,
  }, false)
end

function NetworkController.broadcast_score(p1_score, p2_score, won_player_id)
  if NetworkController.server then
    NetworkController.server:broadcast("score", { p1_score, p2_score, won_player_id }, true)
  end
end

function NetworkController.broadcast_round_reset()
  if NetworkController.server then
    NetworkController.server:broadcast("round", {}, true)
  end
end

-- Returns true on a network tick boundary, which is what rate limits state and input sends.
function NetworkController.tick()
  if pending_client_teardown then
    NetworkController.destroy_client()
  end

  if pending_server_teardown then
    NetworkController.destroy_server()
  end

  local net_tick = false

  if NetworkController.server then
    net_tick = NetworkController.server:tick(App:get_timestep()) or net_tick
  end

  if NetworkController.client then
    net_tick = NetworkController.client:tick(App:get_timestep()) or net_tick
  end

  return net_tick
end

return NetworkController
